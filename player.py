...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self,x,y,filename):
        ...
        img = pygame.image.load("files/walk.png").convert()
        # Create the animations objects / Crée les objets d'animation
        ...
        # Load explosion image / Charge de l'image d'explosion
        img = pygame.image.load("files/explosion.png").convert()
    ...
    def update(self,horizontal_blocks,vertical_blocks):
        if not self.explosion:
            ...
            # This will stop the user for go up or down when it is inside of the box / Cela empêche l'utilisateur de monter ou descendre lorsqu'il est à l'intérieur du cadre.
            ...
            # This will cause the animation to start / Ceci fera démarrer l'animation
...
class Animation(object):
    def __init__(self,img,width,height):
        # Load the sprite sheet / Chargement de la feuille de sprites
        self.sprite_sheet = img
        # Create a list to store the images / Crée une liste pour stocker les images
        self.image_list = []
        self.load_images(width,height)
        # Create a variable which will hold the current image of the list / Crée une variable qui contiendra l'image actuelle de la liste
        self.index = 0
        # Create a variable that will hold the time / Crée une variable qui contiendra le temps
        self.clock = 1
        
    def load_images(self,width,height):
        # Go through every single image in the sprite sheet / Parcourt chaque image de la feuille de sprites.
        for y in range(0,self.sprite_sheet.get_height(),height):
            for x in range(0,self.sprite_sheet.get_width(),width): 
                # load images into a list / charge les images dans une liste
                img = self.get_image(x,y,width,height)
                self.image_list.append(img)

    def get_image(self,x,y,width,height):
        # Create a new blank image / Crée une nouvelle image vierge
        image = pygame.Surface([width,height]).convert()
        # Copy the sprite from the large sheet onto the smaller / Copie le sprite de la grande feuille sur la petite.
        image.blit(self.sprite_sheet,(0,0),(x,y,width,height))
        # Assuming black works as the transparent color / Le noir fonctionne comme couleur transparente
        image.set_colorkey((0,0,0))
        # Return the image / Retourne l'image
        return image
    ...
    def update(self,fps=30):
        ...
        if self.clock in l:
            # Increase index / Indice d'augmentation
            self.index += 1
