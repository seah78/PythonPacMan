...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, horizontal_blocks, vertical_blocks):
        if not self.explosion:
            ...
            # Cela empêche l'utilisateur de monter ou descendre lorsqu'il est à l'intérieur du cadre.
...
class Animation(object):
    def __init__(self, img, width, height):
        ...
        # Crée une variable qui contiendra l'image actuelle de la liste
        self.index = 0
    ...
    def update(self, fps=30):
        step = 30 // fps
        length = range(1, 30, step)
        if self.clock == 30:
            self.clock = 1
        else:
            self.clock += 1

        if self.clock in length:
