...
from ghost import Ghost, Block, Ellipse, enviroment, draw_enviroment
...
class Game(object):
    def __init__(self):
        ...
        self.menu = Menu(
            ("Démarrer", "A propos", "Quitter"), font_color=constant.WHITE, font_size=60
        )
        # Create the player / Créer le joueur
        self.player = Player(32, 128, "files/player.png")
        # Create the blocks that will set the paths where the player can go
        # Crée les blocs qui définiront les chemins où le joueur pourra aller.
            ...
            for j, item in enumerate(row):
                if item == 1:
                    self.horizontal_blocks.add(
                        Block(j * 32 + 8, i * 32 + 8, constant.BLACK, 16, 16)
                    )
                elif item == 2:
        ...
        for i, row in enumerate(enviroment()):
            for j, item in enumerate(row):
                if item != 0:
                    self.dots_group.add(
                        Ellipse(j * 32 + 12, i * 32 + 12, constant.WHITE, 8, 8)
                    )
    ...
    def process_events(self):  # sourcery skip: low-code-quality
        for (
            event
        ) in (
            pygame.event.get()
        ):  # Detection of player's actions / Détection des actions du joueur
            if (
                event.type == pygame.QUIT
            ):  # If user clicked close
                # Si l'utilisateur a cliqué sur quitter, propose le choix entre DEMARRER, A PROPOS, QUITTER
                return True
    ...
    def run_logic(self):
        ...
        # Lorsque la block_hit_list contient un sprite, cela signifie que le joueur a touché un point.
        ...
        if self.game_over:
            if self.about:
                self.display_message(
                    screen, "It is an arcade Game / C'est un eu d'arcade"
                )
            ...
            text = self.font.render(f"Score: {str(self.score)}", True, constant.GREEN)
    ...
    def display_message(self, screen, message, color=constant.RED):
        ...
        posX = (constant.SCREEN_WIDTH / 2) - (width / 2)
        posY = (constant.SCREEN_HEIGHT / 2) - (height / 2)
...
class Menu(object):
    ...
    def display_frame(self, screen):
        for index, item in enumerate(self.items):
            ...
            posY = (constant.SCREEN_HEIGHT / 2) - (t_h / 2) + (index * height)
