...
class Game(object):
    def __init__(self):
        ...
        self.menu = Menu(("Démarrer","A propos","Quitter"),font_color = constant.WHITE,font_size=60)
        ...
        for i,row in enumerate(enviroment()):
            for j,item in enumerate(row):
                if item == 1:
                    self.horizontal_blocks.add(Block(j*32+8,i*32+8,constant.BLACK,16,16))
                elif item == 2:
                    self.vertical_blocks.add(Block(j*32+8,i*32+8,constant.BLACK,16,16))
        ...
        for i, row in enumerate(enviroment()):
            for j, item in enumerate(row):
                if item != 0:
                    self.dots_group.add(Ellipse(j*32+12,i*32+12,constant.WHITE,8,8))
...
class Menu(object):
    state = 0
    def __init__(self,items,font_color=constant.BLACK,select_color=constant.RED,ttf_font=None,font_size=25):
